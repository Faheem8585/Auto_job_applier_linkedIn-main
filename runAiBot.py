                        ...
                        try: 
                            ...
                            try:
                                ...
                                if errored != "stuck" and cur_pause_before_submit:
                                    wait_timeout_seconds = random.randint(60, 120)
                                    decision = pyautogui.confirm(f'AUTO-SUBMITTING IN {wait_timeout_seconds} SECONDS!\n\n1. Please verify your information.\n2. If you edited something, please return to this final screen.\n3. DO NOT CLICK "Submit Application".\n\n\n\n\nYou can turn off "Pause before submit" setting in config.py\nTo TEMPORARILY disable pausing, click "Disable Pause"', "Confirm your information",["Disable Pause", "Discard Application", "Submit Application"], timeout=wait_timeout_seconds * 1000)
                                    if decision == "Discard Application": raise Exception("Job application discarded by user!")
                                    pause_before_submit = False if "Disable Pause" == decision else True
                                    # try_xp(modal, ".//span[normalize-space(.)='Review']")
                                follow_company(modal)
                                elapsed_application_time = time.time() - application_start_time
                                if elapsed_application_time < min_application_time_seconds:
                                    print_lg(f"Application finished too quickly ({elapsed_application_time:.2f}s). Sleeping for {min_application_time_seconds - elapsed_application_time:.2f}s to seem human.")
                                    sleep(min_application_time_seconds - elapsed_application_time)
